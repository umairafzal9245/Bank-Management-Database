...
import DatabaseClasses.DatabaseHandler;
import DatabaseClasses.FileDatabase;
import DatabaseClasses.Oracle;
...
public class BMS 
{
	...
	public void openanaccount()
	{
		...
		System.out.println("Where you want to store data (oracle,mysql,files) : ");
		...
		if(index == -1)
		{
			if(Accounttype.toLowerCase().equals("checking"))
			{
				...
				DatabaseHandler databases = new DatabaseHandler();
				if(dat.equalsIgnoreCase("oracle"))
				{
					databases = new Oracle();
				}
				else if(dat.equalsIgnoreCase("file"))
				{
					databases = new FileDatabase();
				}
				databases.addcustomerstodatabase(newcustomer);
				databases.addaccountstodatabase(newaccount);
			}
